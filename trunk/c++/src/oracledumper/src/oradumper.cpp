...
void OraDumper::DumperInit()
{
	...
	if ( m_strEncodeType.length() > 0 ) {
		...
		if( m_mEncode.count(m_strEncodeType) == 0 ) {
			fprintf(m_fErrorLog, "Error:Unknown Encode Type[%s]!\n", m_strEncodeType.c_str());
			exit(-1);
		} else {
...
void OraDumper::PreDump(long flag)
{
	...
	if (flag != 1) {
		...
		if( m_strMode == "TRUNCATE" ) {
			...
			CStrSplit spt;
			if (spt.Init() == -1) {
				fprintf(m_fErrorLog, "Error:Spt init failed.\n");
				exit(1);
			}
...
void OraDumper::PreLoadFiles()
{
	...
	if( nPos == string::npos ) {
		fprintf(m_fErrorLog, "Error:Table must like owner.table!\n"); 
		exit(-1);
	}
...
ub4 OraDumper::describe_table (struct loadctl *ctlp, text *tablename)
{
	...
	if ((retval = OCIDescribeAny(ctlp->svchp_ctl, ctlp->errhp_ctl, (dvoid *)tablename,
		...
		if (retval == OCI_NO_DATA) {
			fprintf(m_fErrorLog, "NO DATA: OCIDescribeAny on %s\n", tablename);
		} else {/* OCI_ERROR */
			fprintf(m_fErrorLog, "ERROR: OCIDescribeAny on %s\n", tablename);
	...
	if( 0 == strncasecmp(m_strColOrder.c_str(), "ascii", 5)) {
		...
		for(i = 0; i < parmcnt; i++) {
			...
			if( iter == vColName.end() ) {
				fprintf(m_fErrorLog, "Error: Not Found Column[%s]!", spt[i + 1]); 
...
void OraDumper::describe_type(struct loadctl *ctlp,OCIParam *type_parmp)
{
	...
	fprintf(output_fp, "\n\n-----------------\n");
	fprintf(output_fp, "USED-DEFINED TYPE\n-----------------\n");
	...
	fprintf(output_fp, "Oracle Typecode:   %d\n", typecode);
	...
	if (typecode == OCI_TYPECODE_NAMEDCOLLECTION) {
		...
		fprintf(output_fp, "Collection typecode: %d\n", collection_typecode);
	...
	fprintf(output_fp, "Number of methods: %d\n", num_method);
	if (num_method > 0) {
	...
	/* OCI_TYPECODE_VARRAY or OCI_TYPECODE_TABLE */
{
	...
	fprintf(output_fp, "Schema    Type            Length   Type Datatype Elements\n");
	fprintf(output_fp, "_________________________________________________________\n");

	fprintf(output_fp, "%10s%16s%9d%5d%9d%8d\n", schema_name, type_name,
...
void OraDumper::describe_typemethod(struct loadctl *ctlp,OCIParam *method_parmp, text *comment)
{
	...
	fprintf(output_fp, "Has result:        %d\n", has_result);
	if (has_result) {
	...
	fprintf(output_fp, "Number of args:    %d\n", num_arg);
...
void OraDumper::describe_typearg (struct loadctl *ctlp,OCIParam *arglist_parmp, ub1 type, ub4 start,ub4 end)
{
	...
	/* print header */
	fprintf(output_fp, "Name    Pos   Type Datatype Lvl Def Iomode SchName TypeName\n");
	fprintf(output_fp, "________________________________________________________________\n");
...
void OraDumper::describe_typeattr(struct loadctl *ctlp,OCIParam *attrlist_parmp, ub4 num_attr)
{
	...
	fprintf(output_fp, "____________________________________________________________________\n");
	...
	for (pos = 1; pos <= num_attr; pos++) {
		...
		fprintf(output_fp, "%10.*s%10.*s%16.*s%8d%4d%8d%4d%5d\n", namesize, attr_name,
...
void OraDumper::checkerr(OCIError *errhp, sword status)
{
	...
	switch (status)
	{
		...
		case OCI_SUCCESS_WITH_INFO:
			(void) fprintf(m_fErrorLog, "Error - OCI_SUCCESS_WITH_INFO\n");
			break;
		case OCI_NEED_DATA:
			(void) fprintf(m_fErrorLog, "Error - OCI_NEED_DATA\n");
			break;
		case OCI_NO_DATA:
			(void) fprintf(m_fErrorLog, "Error - OCI_NODATA\n");
			break;
		case OCI_ERROR:
			(void) OCIErrorGet ((dvoid *)errhp, (ub4) 1, (text *) NULL, &errcode,
					errbuf, (ub4) sizeof(errbuf), OCI_HTYPE_ERROR);
			(void) fprintf(m_fErrorLog, "Error - %.*s\n", 512, errbuf);
			break;
		case OCI_INVALID_HANDLE:
			(void) fprintf(m_fErrorLog, "Error - OCI_INVALID_HANDLE\n");
			break;
		case OCI_STILL_EXECUTING:
			(void) fprintf(m_fErrorLog, "Error - OCI_STILL_EXECUTE\n");
			break;
		case OCI_CONTINUE:
			(void) fprintf(m_fErrorLog, "Error - OCI_CONTINUE\n");
			break;
		default:
			break;
	}
...
void OraDumper::init_obj_load(struct loadctl *ctlp,struct tbl *tblp,struct obj *objp)
{
	...
	{
		fprintf(m_fErrorLog,"ERROR: expected parmtyp of OCI_PTYPE_LIST, got %d\n",(int)parmtyp);
	}
...
void OraDumper::init_load(struct loadctl *ctlp, struct tbl *tblp, struct sess *sessp)
{
    ...
    for (i = 0, pos = 1, colp = tblp->col_tbl, fldp = tblp->fld_tbl;i < tblp->ncol_tbl;i++, pos++, colp++, fldp++)
    {
        ...
        {
            fprintf(output_fp,"*** ERROR *** bad col name len in column parameter\n");
...
int OraDumper::simple_load(char *line, size_t size)
{
							...
							else if (cvtrv == CONVERT_ERROR)
							{
								...
								if( m_oLoadErr.nLastErrCode == nErrCode )
								{
									...
									if( dotCnt < 320 )
										...
										fprintf(output_fp, "more dots hidden");
                                    */
									dotCnt++;
								...
								else
								{
									...
									{
										//fprintf(m_fErrorLog, "Error[%d]: %d:%s\n", 
									...
									fprintf(m_fErrorLog, "Conversion Error on record %d, column %d\n",(int)err_recnum, (int)cvtBadCoff + 1);
									/* print err msg txt */
							...
							case LOAD_SUCCESS:
								{
									...
									else if (cvtrv == CONVERT_CONTINUE)
									{
										...
										if (startoff != nxtLoadOff) /* sanity*/
											fprintf(m_fErrorLog, "LOAD_SUCCESS/CONVERT_CONTINUE: %ld:%d\n",(long)nxtLoadOff, startoff);
							...
							case LOAD_ERROR:
								{
									...
									m_nRejRows++;
									fprintf(m_fErrorLog, "Error on record %ld\n", (long)err_recnum);
									/* print err msg txt */
	...
	//fprintf(output_fp, "Number of load records processed  = %ld\n\n", (long)m_nLoadRows);
	if (m_notconv != 0)
		fprintf(output_fp, "Number of not conv encode = %ld\n\n", m_notconv);
	...
	for (/* empty */; thiscol < ncols; thiscol++)
	{
		...
		else /* !partial*/
		{
			...
			if (fldp->endpos_fld > recsz )
			{
				if (!prntflg)
				{
					fprintf(m_fErrorLog, "Warning: Max field length, %d for record %d, is greater than the current record size %d.\n",fldp->endpos_fld, (rowoff+1), recsz);
					//printf("%s\n", recp);
					//sleep(2);
					prntflg = 1;
				}
...
void OraDumper::errprint(dvoid *errhp, ub4 htype, sb4 *errcodep)
{
	...
	if (errhp)
	{
		...
		(void) fprintf(m_fErrorLog, "Error[%d] - %.*s\n", errcode, 512, errbuf);
	}
}
...
void OraDumper::checkerr(dvoid *errhp, ub4 htype, sword status, text *note, sb4 code, text *file, sb4 line)
{
	...
	if ((status != OCI_SUCCESS))
		(void) fprintf(m_fErrorLog, "OCI Error %ld occurred at File %s:%ld\n",(long)status, (char *)file, (long)line);
	if (note)
		(void) fprintf(m_fErrorLog, "File %s:%ld (code=%ld) %s\n",(char *)file, (long)line, (long)code, (char *)note);
