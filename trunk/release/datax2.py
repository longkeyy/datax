...
from panguinject import *

engineCmd='''
java -Xmx1024m -Djava.ext.dirs=${libs} -Djava.library.path=${share_library} ${params} -jar ${jar} ${jobdescpath}
'''
editCmd='''
...
if __name__ == '__main__':
    ...
    ##### Yunti2 config #######
    # Set Apsara librarys
    os.environ['LD_LIBRARY_PATH'] = os.environ.get(r'LD_LIBRARY_PATH', r'') + ':/apsara/lib64'
    y2Cfg = getYT2Config(args[0])
    ...
    if not isUrl(args[0]):
        ...
        # inject pangu
        if int(y2Cfg['flag']) == 1:
            injectPangudumper(ctxt['jobdescpath'], y2Cfg)
    else:
        counter = -1
        response = None
        while counter < 3:
    ...
    # report return code
    reportStatus(cmd, retCode)
  
    if retCode == 200:
        retCode = 0
    
    #add by bazhen.csy
    #NOTE just for aliyun
    #we need to try to rerun DataX 
    if retCode != 0:
        sys.exit(2)
    else:
        sys.exit(0)
