...
import com.taobao.datax.common.tools.JobConfGenDriver;
...
public class Engine {
	 ...
	 * 
	 * @param	pluginReg
	 * 			Configurations for {@link Plugin}
	 * 
	 * */
	public Engine(EngineConf engineConf, Map<String, PluginConf> pluginReg) {
		...
		for (PluginConf c : pluginReg.values()) {
			if (c.getPath() == null){
				c.setPath(engineConf.getPluginRootPath());
			}
		}
	}
		...
		while (true) {
			...
			if (sleepCnt % PERIOD == 0) {
				...
				for (NamedThreadPoolExecutor perWriterPool : writerPool) {
					sb.append(String.format("WriterPool %s: Active Threads %d .",
							perWriterPool.getName(), perWriterPool.getActiveCount()));
					logger.info(sb.toString());
					sb.setLength(0);
				}
	...
	private NamedThreadPoolExecutor initReaderPool(JobConf jobConf, StoragePool sp)
			throws Exception {
		...
		if (readerConf.getPath() == null){
			readerConf.setPath(engineConf.getPluginRootPath());
		}
		JarLoader jarLoader = new JarLoader(readerConf.getPath()
				+ readerConf.getJar());
		...
		if (code != 0){
			throw new RerunableException("DataX Reader prepare work failed!");
		}
		logger.info("DataX Reader prepare work ends .");

		logger.info("DataX Reader split work begins .");
		List<PluginParam> readerSplitParams = (List<PluginParam>) readerWorkerForPreAndPost
				.doSplit(sparam);
		logger.info(String.format("DataX Reader splits this job into %d sub-jobs",
				readerSplitParams.size()));
		logger.info("DataX Reader split work ends .");
		
		int concurrency = Math.min(readerJobConf.getConcurrency(), 
				readerSplitParams.size());
		if (concurrency <= 0) {
			concurrency = 1;
			logger.warn("DataX Reader job [concurrency] set to 1 .");
		} else if (concurrency > MAX_CONCURRENCY){
			concurrency  = MAX_CONCURRENCY;
			logger.warn("DataX Reader job [concurrency] set to " + MAX_CONCURRENCY);
		}
	...
	private List<NamedThreadPoolExecutor> initWriterPool(JobConf jobConf,
		...
		for (JobPluginConf dpjc : writerJobConfs) {
			...
			if (writerConf.getPath() == null){
				writerConf.setPath(engineConf.getPluginRootPath());
			}
			JarLoader jarLoader = new JarLoader(writerConf.getPath()
					+ writerConf.getJar());
			Class<?> myClass = jarLoader.loadClass(writerConf.getClassName());

			WriterWorker writerWorkerForPreAndPost = new WriterWorker(writerConf,
					myClass);
			...
			List<PluginParam> writerSplitParams = (List<PluginParam>) writerWorkerForPreAndPost
					.doSplit(writerParam);
			logger.info(String.format("DataX Writer splits this job into %d sub-jobs .",
					writerSplitParams.size()));
			logger.info("DataX Writer split work ends .");

			int concurrency = Math.min(dpjc.getConcurrency(),
					writerSplitParams.size());
			if (concurrency <= 0) {
				concurrency = 1;
				logger.warn("DataX Writer job [concurrency] set to 1 ." );
			} else if (concurrency > MAX_CONCURRENCY){
				concurrency  = MAX_CONCURRENCY;
				logger.warn("DataX Writer job [concurrency] set to " + MAX_CONCURRENCY);
			}
	...
	/**
	 * Program entry </br>>
	 * NOTE: The DataX Process exists code </br>
	 * exit with 0:	Job succeed </br>
	 * exit with 1: Job failed </br>
	 * exit with 2: Job failed, e.g. connetion interrupted, if we try to rerun it in a few seconds, it may succeed.
	 * 
	 * */
	public static void main(String[] args) throws Exception{
		String jobDescFile = null;
		if (args.length < 1) {
			int exitCode = JobConfGenDriver.genConf(FileUtils.readFileToString(
					new File(Constants.PARAMCONFIG), "UTF-8"));
