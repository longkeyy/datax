...
public class Engine {
	...
	private NamedThreadPoolExecutor initReaderPool(JobConf jobConf,
		...
		this.logger.info(String.format("DataX Reader %s try to load path %s .",
		...
		if (concurrency <= 0) {
			...
			logger.warn("DataX Reader job [concurrency] set to "
					+ MAX_CONCURRENCY);
		}
	...
	private List<NamedThreadPoolExecutor> initWriterPool(JobConf jobConf,
		...
		for (JobPluginConf dpjc : writerJobConfs) {
			...
			if (concurrency <= 0) {
				...
				concurrency = MAX_CONCURRENCY;
				logger.warn("DataX Writer job [concurrency] set to "
						+ MAX_CONCURRENCY);
			}
