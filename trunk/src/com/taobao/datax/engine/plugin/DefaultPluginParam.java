...
public class DefaultPluginParam implements PluginParam {
	 ...
	 * NOTE: if key is null, or key does not exists in this {@link PluginParam}, or value is empty, return <code>false</code>
	 * 
	 * @param	key
	 * 			key to be check.
	 * 
	 * @return		
	 * 			if key exists in {@link PluginParam}, return <code>true</code>, others returns <code>false</code>.
	 * */
	...
	public char getCharValue(String key, char defaultValue) {
		...
		char ret;
		try {
			ret = StrUtils.getCharParam(
					StrUtils.replaceString(params.get(key.toLowerCase())),
					defaultValue);
	 ...
	 * @return		
	 * 			Corresponding value in {@link PluginParam} <br> 
	 * 			if the key does not exists in {@link PluginParam}, value is empty, or value is not true/false, return <code>false</code> instead.
	 */
	@Override
	...
	public int getIntValue(String key, int defaultValue) {
		...
		int ret = 0;
		try {
			ret = Integer.valueOf(StrUtils.replaceString(this.params.get(key)));
	...
	public double getDoubleValue(String key, double defaultValue) {
		...
		double ret = 0.0;
		try {
			ret = Double.valueOf(StrUtils.replaceString(this.params.get(key)));
		} catch (NumberFormatException e) {
			LOG.warn(e.getMessage());
			ret = defaultValue;
		} catch (Exception e) {
	 ...
	 * 
	 * @return		
	 * 			all keys exists in {@link PluginParam}.
	 * */
	@Override
