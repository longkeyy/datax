...
public class RAMStorage extends Storage {
	...
	private long lastRRefusedTime = System.currentTimeMillis();

	private byte[] rLock = new byte[0];
		...
		try {
			while (mars.offer(lines, size, waitTime, TimeUnit.MILLISECONDS) == false) {
				synchronized (rLock) {
					...
					if (t - lastRRefusedTime > 1000) {
						getStat().incLineRRefused(1);
						if (log.isDebugEnabled()) {
							log.debug("RAMStorage(" + getStat().getId()
									+ ") refused: " + info() + ", limit="
									+ destructLimit + ", count="
					...
					if (destructLimit > 0
							&& getStat().getLineRRefused() > destructLimit) {
						log.warn("RAMStorage close.");
						setPushClosed(true);
						break;
					}
				}
