...
public class StoragePool {
	...
	public StoragePool(JobConf jobConf, EngineConf engineConf, int period) {
		for (JobPluginConf jpc : jobConf.getWriterConfs()) {
			Storage s = StorageFactory
					.product(engineConf.getStorageClassName());
			s.init(jpc.getId(), engineConf.getStorageLineLimit(),
					engineConf.getStorageByteLimit(), jpc.getDestructLimit());
