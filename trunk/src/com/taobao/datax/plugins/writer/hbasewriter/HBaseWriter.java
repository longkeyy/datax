...
public class HBaseWriter extends Writer {
	...
	public int init() {
		...
		autoflush = param.getBoolValue(ParamKey.autoFlush, true);
		bufferSize = param
				.getIntValue(ParamKey.bufferSize, DEFAULT_BUFFER_SIZE);

		/* if user does not set rowkey index, use 0 for default. */
		rowkeyIndex = param.getIntValue(ParamKey.rowkey_index, 0);
		...
		try {
			...
			if (null == this.proxy || !this.proxy.check()) {
				throw new RerunableException("HBase Client initilize failed .");
			}
	...
	public int startWrite(LineReceiver receiver) {
		...
		while ((line = receiver.getFromReader()) != null) {
			try {
				...
				if (columnIndexes.length > fieldNum) {
					String msg = String.format("Key length error @%s .",
							line.toString('\t'));
					logger.error(msg);
					this.getMonitor().lineFail(msg);
					continue;
				}
