...
public class HdfsWriter extends Writer {
	...
	public int prepare(PluginParam param) {
		...
		String ugi = param.getValue(ParamsKey.HdfsWriter.ugi, null);
		String prefixname = param.getValue(ParamsKey.HdfsWriter.prefixname,
				"prefix");
		delMode = param.getValue(ParamsKey.HdfsWriter.delMode, this.delMode);
		concurrency = param.getIntValue(ParamsKey.HdfsWriter.concurrency, 1);
		hadoop_conf = param.getValue(ParamsKey.HdfsWriter.hadoop_conf, "");
	...
	@Override
	public int init() {
		FIELD_SPLIT = param.getCharValue(ParamsKey.HdfsWriter.fieldSplit,
				FIELD_SPLIT);
		ENCODING = param.getValue(ParamsKey.HdfsWriter.encoding, ENCODING);
		LINE_SPLIT = param.getCharValue(ParamsKey.HdfsWriter.lineSplit,
				LINE_SPLIT);
		searchChars[0] = FIELD_SPLIT;
		searchChars[1] = LINE_SPLIT;
		BUFFER_SIZE = param.getIntValue(ParamsKey.HdfsWriter.bufferSize,
				BUFFER_SIZE);
		delMode = param.getValue(ParamsKey.HdfsWriter.delMode, this.delMode);
		nullChars = param.getValue(ParamsKey.HdfsWriter.nullChar, "")
				.toCharArray();
		hadoop_conf = param.getValue(ParamsKey.HdfsWriter.hadoop_conf, "");
		...
		if (fs == null) {
			...
			throw new RerunableException(
					"HdfsWriter Initialize file system failed.");
		}
		if (dir != null) {
			p = new Path(dir);
		} else {
			...
			throw new RerunableException("Can't find the param ["
					+ ParamsKey.HdfsWriter.dir + "] in hdfs-writer-param.");
		}

		String filetype = param.getValue(ParamsKey.HdfsWriter.fileType, "TXT");
	...
	public int connect() {
		...
		try {
			...
			log.debug("the HdfsWriter is connected.");
			return PluginStatus.SUCCESS.value();
		} catch (Exception ex) {
			closeAll();
			ex.printStackTrace();
	...
	public int startWrite(LineReceiver receiver) {
		...
		log.debug("HdfsWriter is starting");
		try {
	...
	private void closeAll() {
		try {
			...
			log.debug("HdfsWriter is closed.");
		} catch (Exception e) {
			throw new RerunableException(String.format(
	...
	class DfsWriterSequeueFileStrategy implements DfsWriterStrategy {
		...
		public DfsWriterSequeueFileStrategy() {
			...
			String cType = getParam().getValue(
					ParamsKey.HdfsWriter.compressionType, "NONE");
		...
		public void open() {
			try {
				if ("1".equals(delMode) && fs.exists(p))
					throw new RerunableException("the file [" + p.getName()
							+ "] already exists. ");

				String codecClassName = getParam().getValue(
						ParamsKey.HdfsWriter.codecClass,
						"org.apache.hadoop.io.compress.DefaultCodec");
				keyClassName = getParam().getValue(
						ParamsKey.HdfsWriter.keyClass,
						"org.apache.hadoop.io.Text");
				valueClassName = getParam().getValue(
						ParamsKey.HdfsWriter.valueClass,
						"org.apache.hadoop.io.Text");
				...
				keyFieldIndex = param.getIntValue(
						ParamsKey.HdfsWriter.keyFieldIndex, this.keyFieldIndex);

				if (keyClassName.toLowerCase().indexOf("null") == -1
						&& (keyFieldIndex >= 0))
					keySetMethod = keyClass.getMethod(
							"set",
							new Class[] { DfsUtils.getTypeMap().get(
									keyClassName) });
				if (valueClassName.toLowerCase().indexOf("null") == -1)
				...
				throw new RerunableException(ex);
		...
		private Object adapterType(String field, String typename) {
			Object target = null;
			if (typename.toLowerCase().indexOf("null") > -1) {
				target = null;
			} else if (typename.toLowerCase().indexOf("text") > -1) {
				target = field;
			} else if (typename.toLowerCase().indexOf("long") > -1) {
				target = Long.parseLong(field);
			} else if (typename.toLowerCase().indexOf("integer") > -1) {
				target = Integer.parseInt(field);
			} else if (typename.toLowerCase().indexOf("double") > -1) {
				target = Double.parseDouble(field);
			} else if (typename.toLowerCase().indexOf("float") > -1) {
	...
	// ParamsKey.HdfsWriter.hiveTableswitch, "false");
	...
	// throw new RerunableException("md is null");
	// }
	//
	// String tableName = getTableName(param, md);
	// String fileType = getFileType(param);
	// String dataLocal = param.getValue(ParamsKey.HdfsWriter.dir, null); //
	...
	// ParamsKey.HdfsWriter.fieldSplit, "\\t");
	// String line_split = param.getValue(
	// ParamsKey.HdfsWriter.lineSplit, "\\n");
	...
	// e.printStackTrace();
	// throw new RerunableException("Hive Driver Not Found");
	// } catch (SQLException e) {
	// System.out.println("sql failed");
	// e.printStackTrace();
	// throw new RerunableException("sql failed");
	// } catch (Exception e) {
	// e.printStackTrace();
	// throw new RerunableException();
	// } finally {
	...
	// ParamsKey.HdfsWriter.partitionNames, "").trim();
	// String origParValueInfo = param.getValue(
	// ParamsKey.HdfsWriter.partitionValues, "").trim();
	...
	// Connection con;
	// String hiveServer = param.getValue(ParamsKey.HdfsWriter.hiveServer,
	// "10.232.128.67");
	// String hivePort = param.getValue(ParamsKey.HdfsWriter.hiveServerPort,
	// "10000");
	...
	// String profileType = param.getValue(ParamsKey.HdfsWriter.fileType,
	// "TXT");
	...
	// String tableName = param.getValue(ParamsKey.HdfsWriter.tableName, null);
