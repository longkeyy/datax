...
public class OracleReaderRowidSplitter extends Splitter {
	...
	@Override
	public int init() {
		this.tables = this.param.getValue(ParamsKey.OracleReader.tables);
		this.columns = this.param.getValue(ParamsKey.OracleReader.columns, "*");
		this.where = this.param.getValue(ParamsKey.OracleReader.where, "");
		this.schema = this.param.getValue(ParamsKey.OracleReader.schema);
		this.rowidSplitMode = this.param.getIntValue(
				ParamsKey.OracleReader.splitMod, this.rowidSplitMode);
	...
	public List<PluginParam> split() {
		...
		for (String tablet : tablets) {
			...
			if (null == partitions || 0 == partitions.size()) {
				if (1 == this.rowidSplitMode) {
					sql = format(this.ORACLE_ROWID_SPLIT_PATTERN,
							schema.toUpperCase(), tablet.toUpperCase());
				} else {
					sql = format(this.ORACLE_ROWID_NTILE_SPLIT_PATTERN,
							schema.toUpperCase(), tablet.toUpperCase());
				}
				rowids.addAll(this.analyzeRowids(tablet, sql));
			} else {
				for (String partition : partitions) {
					sql = format(this.ORACLE_ROWID_PARTITION_SPLIT_PATTERN,
							schema.toUpperCase(), tablet.toUpperCase(),
		...
		for (Map<RowidKey, String> perRowid : rowids) {
			...
			//logger.info(sql);
			param.putValue(ParamsKey.OracleReader.sql, sql);
			params.add(param);
		}
	...
	private List<String> analyzePartitions(String table) {
		...
		String partitionsSql = format(this.QUERY_PARTITIONS_PATTERN,
	...
	private String genRowidSql(String table, String minRowid, String maxRowid) {
		String sql = "";
		String tail = format(this.SQL_ROWID_BETWEEN_PATTERN, minRowid, maxRowid);
		if (StringUtils.isBlank(this.where)) {
			sql = format(SQL_WITHOUT_WHERE_PATTERN, this.columns, this.schema
					+ "." + table)
					+ " where " + tail;
