...
package com.taobao.datax.plugins.reader.httpreader;
...
public class HttpReader extends Reader {
	...
	public int init(){
		...
		this.FIELD_SPLIT = param.getValue(ParamsKey.HttpReader.fieldSplit,
				"\t");
		this.ENCODING = param.getValue(ParamsKey.HttpReader.encoding,
				"UTF-8");
		this.nullString = param.getValue(ParamsKey.HttpReader.nullString,
				"");
		this.httpURL = param.getValue(ParamsKey.HttpReader.httpURLs);
	...
	public int startRead(LineSender sender) {
		...
		try {
			...
			try {
				...
				throw new RerunableException(message);
			}
