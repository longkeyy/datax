...
public class MysqlReaderSplitter extends Splitter {
	...
	@Override
	public int init() {
		String tables = param.getValue(ParamsKey.MysqlReader.tables);
	...
	private List<PluginParam> splitParamsList(PluginParam iParams) {
		...
		String where = iParams.getValue(ParamsKey.MysqlReader.where, null);
		String columns = iParams.getValue(ParamsKey.MysqlReader.columns,
				"*");
		for (String table : tableNames) {
			...
			oParams.putValue(ParamsKey.MysqlReader.sql, sql);
