...
public class SqlServerSqlGenerator {
	...
	public List<String> generate() {
		...
		List<String> tables = SplitUtils.splitTables(iParam.getValue(ParamsKey.SqlServerReader.tables));
	...
	private String parseWhere() {
		if (iParam.hasValue(ParamsKey.SqlServerReader.where)) {
			return "where " +  iParam.getValue(ParamsKey.SqlServerReader.where);
		} else {
			return "";
		}
	}
	...
	private String parseCols() {
		if (iParam.hasValue(ParamsKey.SqlServerReader.columns)) {
			return  iParam.getValue(ParamsKey.SqlServerReader.columns);
		} else {
			return "*";
		}
	}
